...
import (
	...
	"github.com/fortinetdev/terraform-provider-fortiweb/web-sdk/request"
	"strconv"
)
...
func NewClient(auth *auth.Auth, client *http.Client) (*FortiSDKClient, error) {
	...
	if err != nil {
		...
		c.Fv = "Unknown (fortiweb non-global user can't get version)"
	} else {
		c.Fv = v
	}
...
// Logout logs out from the FortiWeb device
func (c *FortiSDKClient) Logout() error {
	return Logout(c)
}
