...
import (
	...
	"github.com/fortinetdev/terraform-provider-fortiweb/web-sdk/auth"
	//forticlient "github.com/fortinetdev/forti-sdk-go/fortiweb/sdkcore"
	forticlient "github.com/fortinetdev/terraform-provider-fortiweb/web-sdk/sdkcore"
)
...
func set_log() {
	f, err := os.OpenFile("/tmp/web", os.O_RDWR|os.O_CREATE|os.O_APPEND, 0666)
	if err != nil {
...
func (c *Config) CreateClient() (interface{}, error) {
	...
	if !bFOSExist {
		return nil, fmt.Errorf("FortiWEB at least one of their hostnames should be set")
	}
...
func bFortiOSHostnameExist(c *Config) bool {
	if c.Hostname == "" {
		if os.Getenv("FORTIWEB_ACCESS_HOSTNAME") == "" {
			return false
		}
	}
...
func createFortiOSClient(fClient *FortiClient, c *Config) error {
	config := &tls.Config{}
	auth := auth.NewAuth(c.Hostname, c.Token, c.Username, c.Password, c.CABundle, c.CABundleContent, c.PeerAuth, c.CaCert, c.ClientCert, c.ClientKey, c.Vdom, c.HTTPProxy, c.HTTPSProxy, c.NOProxy)
	...
	if auth.CABundle != "" {
		if auth.CABundleContent != "" {
			return fmt.Errorf("\"cabundlefile\" and \"cabundlecontent\" could not exist at the same time! Please only configure one of them. If you are not configure \"cabundlefile\", please check the environment variable \"FORTIWEB_CA_CABUNDLE\".")
		}
	...
	// If username and password are provided, use Login function to get token and cookies
	if auth.Username != "" && auth.Password != "" {
		// Call Login function to get token, cookies, and CSRF token
		////token, cookies, _, csrfToken, err := forticlient.Login(auth.Hostname, auth.Username, auth.Password, "443")
		////if err != nil {
		////	return fmt.Errorf("Error logging in: %v", err)
		////}
		// Set the retrieved values in the auth struct
		////auth.Token = token
		////auth.Cookies = cookies
		////auth.CSRFToken = string(csrfToken)
	}
