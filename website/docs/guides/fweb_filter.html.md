...
data "fortiweb_firewall_policylist" sample1 {
  filter = "schedule==always"
}

output sample1_output {
  value = data.fortiweb_firewall_policylist.sample1.policyidlist
}

# To display interfaces that do not have the name "port1", use:
data "fortiweb_system_interfacelist" sample2 {
  filter = "name!=port1"
}

output sample2_output {
  value = data.fortiweb_system_interfacelist.sample2.namelist
}
...
data "fortiweb_firewall_policylist" sample3 {
  filter = "schedule==always,schedule==once"
}

output sample3_output {
  value = data.fortiweb_firewall_policylist.sample3.policyidlist
}

# To display firewall policies with a schedule of "always" AND an action of "accept", use:
data "fortiweb_firewall_policylist" sample4 {
  filter = "schedule==always&action==accept"
}

output sample4_output {
  value = data.fortiweb_firewall_policylist.sample4.policyidlist
}

# To display firewall policies with a schedule of "always" AND an action of either "accept" or "deny", use:
data "fortiweb_firewall_policylist" sample5 {
  filter = "schedule==always&action==accept,action==deny"
}

output sample5_output {
  value = data.fortiweb_firewall_policylist.sample5.policyidlist
}

# To display firewall policies with policyid > 3, use:
data "fortiweb_firewall_policylist" sample6 {
  filter = "policyid>3"
}

output sample6 {
  value = data.fortiweb_firewall_policylist.sample6.policyidlist
}

# To display firewall addresses with name!=google-play and create an addrgrp object with the list, use:
data "fortiweb_firewall_addresslist" sample7 {
  filter = "name!=google-play"
}

output sample7 {
  value = data.fortiweb_firewall_addresslist.sample7.namelist
}

resource "fortiweb_firewall_addrgrp" grp1 {
  name = "addrgrptest1"

  dynamic_sort_subtable = "true"

  dynamic "member" {
    for_each = toset(data.fortiweb_firewall_addresslist.sample7.namelist)
    content {
      name = member.key
    }
  }
