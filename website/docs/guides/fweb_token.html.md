...
subcategory: ""
layout: "fortiweb"
page_title: "Generate an API token for FortiWEB"
description: |-
  Generate an API token for FortiWEB.
---
...
*Note: The steps below must be performed from the FortiWEB GUI as an Administrator with the super_admin profile and are customized for this tutorial.*
...
* Step 1: Create an Administrator profile
    ...
    On the FortiWEB GUI, select `System > Admin Profiles > Create New`. Populate the fields according to your environment.

* Step 2: Create the REST API Admin

    Use the steps below to create the FortiWEB API admin.

	On the FortiWEB GUI, select `System > Administrators > Create New > REST API Admin`.
...
For more about the configuration of token, please refer to the `System Management` -> `Manage administrator users` -> `Create REST API administrator users` chapters of the `FortiWEB Handbook - Administration Guide`.
